...
```mermaid
flowchart TD
Specific["Specific<br/>(Not a global goal)"] --> Measurable["Measurable<br/>(We can measure it)"] --> Attainable["Attainable<br/>(We can achieve it)"] --> Relevant["Relevant<br/>(It connects with our values and vision)"] --> Timely["Timely<br/>(It has a due date!)"]
```
...
```mermaid
flowchart LR
    Myth1["❌ I do not have time"] --> Truth1["✅ You have, but you are not prioritizing"]
    Myth2["❌ Working more hours is the solution"] --> Truth2["✅ We need constancy not excess"]
